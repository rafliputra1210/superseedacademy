...
<head>
    ...
    <style>
        ...
        /* Wrapper per kartu agar tombol bisa diposisikan */
        .card-wrapper {
            display: flex;
            flex-direction: column;
            align-items: center;
        ...
        .btn-cetak-satuan:hover {
            opacity: 0.88;
            transform: translateY(-1px);
        }
        .btn-cetak-satuan:active {
...
<body>
    ...
    <!-- Container untuk Semua KARTU ID ATHLETE -->
    <div class="cards-container">
        @foreach($athletes as $athlete)
        <div class="card-wrapper">
            <!-- Tombol Cetak Satuan -->
            <button class="btn-cetak-satuan no-print" onclick="cetakSatuan({{ $athlete->id }})">
                🖨️ Cetak Satuan
            </button>
        <div class="id-card" id="card_{{ $athlete->id }}">
            ...
            <div class="barcode-area">
                ...
                <svg id="barcode_{{ $athlete->id }}" class="barcode-svg" data-barcode-value="{{ $athlete->kode_barcode }}"></svg>
    ...
    <script>
        ...
        function cetakSatuan(athleteId) {
            var cardEl = document.getElementById('card_' + athleteId);
            if (!cardEl) return;
            ...
            printWin.document.write(`
                ...
                <body>
                    ...
                    <script>
                        document.addEventListener('DOMContentLoaded', function() {
                            var barcodeEl = document.querySelector('svg[id^="barcode_"]');
                            if (barcodeEl) {
                                var barcodeId = barcodeEl.id;
                                var val = barcodeEl.getAttribute('data-barcode-value');
                                if (val) {
                                    JsBarcode('#' + barcodeId, val, {
                                        format: 'CODE128', width: 1.5, height: 35,
                                        displayValue: true, fontSize: 10,
                                        fontOptions: 'bold', lineColor: '#0f172a', margin: 2
                                    });
                                }
                            }
